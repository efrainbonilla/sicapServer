...
define(function () {
	...
	function InterceptorAdmin (RestangularProvider) {
	    ...
	    RestangularProvider.addResponseInterceptor(function (data, operation, what, url, response, deferred) {
			...
			if (operation === 'get' && what === 'comercios' && angular.isArray(data.mcom)) {
				var mcom = [];
				angular.forEach(response.data.mcom, function(item) {
					mcom.push(item.tcom.tcom_id);
				});
				response.data.mcom = mcom;
			}
			if (operation === 'get' && what === 'productos' && angular.isArray(data.prestc)) {
				var prestc = [];
				angular.forEach(response.data.prestc, function(item) {
					prestc.push(item.med.med_id);
				});
				response.data.prestc = prestc;
			}

...
define(function() {
	...
	var HandlePrintController = function($scope, $document, appConfig) {

		var title = appConfig.RPT_PAGE_TITLE;
		...
		$scope.printPopup = function(title, subtitle, text, withoutRifAndLogus, withoutStyles) {
			...
			if (!withoutRifAndLogus) {
				...
				body += "<td><img border='0' src='/img/"+appConfig.RPT_URI_LOGO_LEFT+"' width='100' height='100'></td>";
				body += "<td style='text-align:center'>"+title+"</td>";
				body += "<td><img border='0' src='/img/"+appConfig.RPT_URI_LOGO_RIGHT+"' width='100' height='100' style='float:right'></td>";
				body += "</tr></tbody></table>";
	...
	HandlePrintController.$inject = ['$scope', '$document', 'appConfig'];

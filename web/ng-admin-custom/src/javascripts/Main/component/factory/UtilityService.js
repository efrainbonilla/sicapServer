	...
	function UtilityService($rootScope, RestWrapper, moment) {
		return {
			...
			apiRubroProducto: function($item, $model, $limit) {
				return RestWrapper.getList({}, 'categorias', '/api/categorias?limit=' + ($limit || '500000'));
			},

			apiComerciante: function ($item, $model, $limit) {
				return RestWrapper.getList({}, 'comerciantes', '/api/regdiarios/comerciantes?limit=' + ($limit || '5000'));
			},

			apiProductoFacturas: function($item, $model, $limit) {
				return RestWrapper.getList({}, 'facturas', '/api/productos/facturas?limit=' + ($limit || '50000'));
			}
		};
	}

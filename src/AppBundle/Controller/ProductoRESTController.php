...
class ProductoRESTController extends VoryxController
{
    ...
    public function postAction(Request $request)
    {
        ...
        $this->prestcEntity($request, $entity);
    ...
    public function prestcEntity(Request $request, Producto $entity)
    {
        if (is_array($request->request->get('prestc'))) {
            $prestc = $request->request->get('prestc');
            $em = $this->getDoctrine()->getManager();
            $entityPrestc = $em->getRepository('AppBundle:ProdPrestacion')->findBy(array('prod' => $entity->getProdId()));

            $medIds = array();
            foreach ($entityPrestc as $key => $enti) {
                $medId = $enti->getMed()->getMedId();
                if (in_array($medId, $prestc)) {
                    if (($key = array_search($medId, $prestc)) !== false) {
                        unset($prestc[$key]);
                    }
                } else {
                    $medIds[] = $enti;
            ...
            //add entity
            foreach ($prestc as $key => $value) {
                $entityMedida = $em->getRepository('AppBundle:Medida')->find($value);
                if ($entityMedida) {
                    $entityProdPrestacion = new ProdPrestacion();
                    $entityProdPrestacion->setMed($entityMedida);
                    $entity->addPrestc($entityProdPrestacion);
                }
            }
        }
    }
